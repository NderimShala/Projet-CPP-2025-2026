...
ApplicGarageWindow::ApplicGarageWindow(QWidget *parent) : QMainWindow(parent),ui(new Ui::ApplicGarageWindow)
{
    ...
    //AFFICHER ADMIN AU DEMARRAGE
    clearTableEmployees();
    const auto& listeEmployees = Garage::getInstance().getEmployees();
    ...
    Garage::getInstance().importModelsFromCsv("../models.csv");
    for(int i = 0; i < Garage::getInstance().getNbrModelsCsv(); i++)
    {
    ...
    Garage::getInstance().importOptionsFromCsv("../options.csv");
    for(int i=0; i<Garage::getInstance().getNbrOptionsCsv(); i++)
    {
...
void ApplicGarageWindow::on_actionAddEmployee_triggered()
{
    ...
    while(type != 1 && type != 0)
    {
        type = this->dialogPromptInt("Nouvel employé","Fonction (0=administratif,1=vendeur) :");
        if (type != 1 && type != 0)
        {
            dialogError("Erreur", "Valeur possibles 0 ou 1");
            type = -1;
        }
    }
    ...
    clearTableEmployees();
    string t;
    if(type == 0) t = "Administratif";
    else if(type == 1)  t = "Vendeur";

    Garage::getInstance().addEmployee(lastName, firstName, login, t);
    const auto& listeEmployees = Garage::getInstance().getEmployees();
    for(auto it = listeEmployees.begin(); it != listeEmployees.end(); it++)
    {
        addTupleTableEmployees(it->tuple());
...
void ApplicGarageWindow::on_actionDeleteEmployeeById_triggered()
{
    ...
    int Id = dialogPromptInt("Delete Employee", "Quel est l'ID ?");
...
void ApplicGarageWindow::on_actionDeleteEmployeeByIndex_triggered()
{
    ...
    if(indice == -1)
    {
        dialogError("Erreur", "Aucun employé n'est sélectionné");
        return;
    }
    Garage::getInstance().deleteEmployeeByIndex(indice);
    clearTableEmployees();
    const auto& listeEmployees = Garage::getInstance().getEmployees();
    for (auto it = listeEmployees.begin(); it != listeEmployees.end(); it++)
    {
...
void ApplicGarageWindow::on_actionAddClient_triggered()
{
    ...
    const auto& listeClients = Garage::getInstance().getClients();
    for(auto it = listeClients.begin(); it != listeClients.end(); it++)
    {
...
void ApplicGarageWindow::on_actionDeleteClientById_triggered()
{
    ...
    int Id = dialogPromptInt("Suppression Client", "ID à supprimer : ");
...
void ApplicGarageWindow::on_actionLogin_triggered()
{
    ...
    const auto& listeEmployees = Garage::getInstance().getEmployees();
    auto it = listeEmployees.begin();
    while(it != listeEmployees.end() && it->getLogin() != login) ++it;

    if(it == listeEmployees.end())
    {
        dialogError("Erreur", "Login introuvable");
        return;
    }
    Garage::getInstance().idLoggedEmployee = it->getId();
    try
    {
        string mdp = it->getPassword(); //peut throw si pas de mdp
        string testMdp = dialogPromptText("Connexion", "Mot de passe :");
        if(testMdp != mdp) 
        {
            dialogError("Erreur", "Mot de passe Incorrect");
            Garage::getInstance().idLoggedEmployee = -1;
            return;
        }
    }
    catch(const PasswordException p)
    {
        if(p.getPassCode() != 4)
        {
            dialogError("Erreur", p.getMessage().c_str());
            Garage::getInstance().idLoggedEmployee = -1;
            return;
        }

        dialogMessage("Pas de mot de passe", "Encodez un nouveau mot de passe");
        string NewMdp = dialogPromptText("Nouveau", "Mot de passe :");
        
        Employee ItAvecMdp = *it;
        try
        {
            ItAvecMdp.setPassword(NewMdp); //peut throw si mauvais encodage de format de mdp
        }
        ...
        Garage::getInstance().updateEmployee(ItAvecMdp);

        dialogMessage("Mot de passe", "Mot de passe enregistré, veuillez vous reconnecter");
        Garage::getInstance().idLoggedEmployee = -1;
        setRole(0);
        return;
    }
    ...
    if(role == "Administratif") setRole(1);
    else if(role == "Vendeur") setRole(2);
    else setRole(0);
    ...
    clearTableClients();
    const auto& listeClients = Garage::getInstance().getClients();
    for (auto itc = listeClients.begin(); itc != listeClients.end(); ++itc)
    {
        addTupleTableClients(itc->tuple());
    }
...
void ApplicGarageWindow::on_actionResetPassword_triggered()
{
    ...
    int id = Garage::getInstance().idLoggedEmployee;
    Employee e = Garage::getInstance().findEmployeeById(id);
    e.resetPassword();
    Garage::getInstance().updateEmployee(e);
    dialogMessage("INFO", "le mot de passe a été réinitialisé");
}
...
void ApplicGarageWindow::on_pushButtonSelectModel_clicked()
{
    ...
    if(index == -1) dialogError("Erreur", "Aucun modèle sélectionné");
    else
    {
...
void ApplicGarageWindow::on_pushButtonRemoveOption_clicked()
{
    ...
    else
    {
        ...
        if(o == nullptr) 
        {
            dialogError("Erreur", "Pas d'option à cet indice");
            return;
        }
        try //si jamais je ne coche pas de case ou je coche une case vide 
        {
            
            projet.removeOption(o->getCode());

            // Après suppression, Car::removeOption() décale les options vers la gauche.
            // On redessine donc les 5 lignes pour garder l'UI synchronisée.
            for(int i = 0; i < 5; i++)
            {
                Option* opt = projet[i]; //recup l'option d'indice i
...
void ApplicGarageWindow::on_pushButtonOpenProject_clicked()
{
    ...
    if(nom.empty()) 
    {
        dialogError("Erreur", "Aucun nom de projet");
        return;
    }
    ...
    ifstream testfile((nom + ".xml").c_str());
    if (!testfile.good()) {
        dialogError("Erreur", ("Fichier " + nom + ".xml introuvable").c_str());
        return;
    }
    try
    {
    ...
    {
        dialogError("Erreur", e.getMessage().c_str()); //c_str() convertit string de exception en const char* necessaire pour dialogError
        return;
    }
    ...
    for(int i = 0; i<5; i++) //mettre à jour les options sur l'interface graphique
    {
...
void ApplicGarageWindow::on_pushButtonNewProject_clicked()
{
    ...
    clearTableOption();
    setCurrentProjectName("");
    setModel("", 0, -1, 0.0, "image_vide.png");
    setPrice(0.0);
}
