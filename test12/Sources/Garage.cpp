...
{
    //admin par défaut
    employees.insert(Employee("ADMIN", "ADMIN", Actor::currentId, "ADMIN", "Administratif"));
    Actor::currentId++;
}
...
void Garage::updateEmployee(const Employee& updateEmployee)
{
    for (auto it = employees.begin(); it != employees.end(); it++)
    {
        if(it->getId() == updateEmployee.getId())
        {
            employees.erase(it);
            break;
        }
    }
    employees.insert(updateEmployee);
}

void Garage::save()
{
    ofstream fconfig("../datas/config.dat", ios::binary | ios::out | ios::trunc);

    

    fconfig.write((char*)&Actor::currentId, sizeof(Actor::currentId));
    int n = employees.size();
    fconfig.write((char*)&n, sizeof(n));

    for(auto it = employees.begin(); it != employees.end(); it++)
    {
        Credentials cred;
        strcpy(cred.login, it->getLogin().c_str());
        try
        {
            strcpy(cred.password, it->getPassword().c_str());
        }
        catch(PasswordException& p)
        {
            if(p.getPassCode() == 4) strcpy(cred.password, ""); // 4 = nopassword
        }
        cred.crypt();
        fconfig.write((char*)&cred, sizeof(Credentials));
    }
    try
    {
        XmlFileSerializer<Client> fclients("../datas/clients.xml", XmlFileSerializer<Client>::WRITE, "Clients");
        for(auto it = clients.begin(); it != clients.end(); it++)
        {
            fclients.write(*it);
        }
        XmlFileSerializer<Employee> femployees("../datas/employees.xml", XmlFileSerializer<Employee>::WRITE, "Employees");
        for(auto it = employees.begin(); it != employees.end(); it++)
        {
            femployees.write(*it);
        }
...
void Garage::load()
{
    clients.clear();
    employees.clear();
    XmlFileSerializer<Client> *fclient = nullptr;
    XmlFileSerializer<Employee> *femployees = nullptr;

    vector<Credentials> creds;

    try
    {
        fclient = new XmlFileSerializer<Client>("../datas/clients.xml", XmlFileSerializer<Client>::READ);
        cout << "Nom du fichier : " << fclient->getFilename() << endl;
        cout << "Nom de Collection : " << fclient->getCollectionName() << endl;
    }
    catch(const XmlFileSerializerException& e)
    {
        cout << "Erreur : " << e.getMessage() << " (code = " << e.getCode() << ")" << endl;
    ...
    ifstream fconfig("../datas/config.dat", ios::binary);
    if(fconfig)
    {
        fconfig.read((char*)&Actor::currentId, sizeof(Actor::currentId));
        int n = 0;
        fconfig.read((char*)&n, sizeof(n));

        for(int i = 0; i < n; i++)
        {   
            Credentials tmp;
            fconfig.read((char*)&tmp, sizeof(Credentials));
            tmp.decrypt();
    ...
    try
    {
        femployees = new XmlFileSerializer<Employee>("../datas/employees.xml", XmlFileSerializer<Employee>::READ);
        cout << "Nom du fichier : " << femployees->getFilename() << endl;
        cout << "Nom de Collection : " << femployees->getCollectionName() << endl;
    }
    catch(const XmlFileSerializerException& e)
    {
    ...
    if(femployees != nullptr)
    {
        ...
        while(!end)
        {
            try
            {
                Employee val = femployees->read();
                for(int i = 0; i < sizeof(creds); i++)
                {
                    if(strcmp(creds[i].login, val.getLogin().c_str()) == 0)
                    {
                        if(strcmp(creds[i].password, "") != 0)
                        {
                            val.setPassword(creds[i].password);
                        }
                        break;
                    }
                }
                employees.insert(val);
